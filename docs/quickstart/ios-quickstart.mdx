...
component for that: `SwiftUIVideoView`.
...
struct RoomScreen: View {
  ...
  var body: some View {
    VStack {
      ForEach(contentViewController.participants) { participant in
        SwiftUIVideoView((participant.videoTrackContext?.track as? VideoTrack)!)
...
the room. We have to handle this by implementing some more methods from the
`JellyfishClientListener` protocol:
