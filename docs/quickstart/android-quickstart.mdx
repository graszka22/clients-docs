...
- Android Studio IDE, Android device
...
![Scanning qr code](./img/scan_qr.gif)
...
Finally we can write code to actually connect to the backend. It will be the
...
and local user id. We're going to store information about peers in `MainViewModel`:
...
// TODO: to jest głupie ze user musi się tym zajmować zeby ustawić renderer i
jeszcze zwalnianiem tego, user powinien móc po prostu dać video tracka do tego
view i wszystko
...
`onTrackRemoved()` is called when a track is no longer streamed. One peer can
have multiple tracks (also multiple video tracks!), but for this tutorial we'll
assume it's always one video track.
...
And in `MainViewModel` we'll add `disconnect()` function:
...
It's in React Native, but actually React Native SDK uses Android SDK under the hood!
