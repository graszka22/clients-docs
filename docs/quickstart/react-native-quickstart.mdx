...
On Android add to your `AndroiManifest.xml`:
...
We also suggest setting background mode to `audio` so that the app doesn't disconnect when it's in background:
...
example though we skip that and just copy it from the dashboard to the app for
your convenience. Here is how to do it:
...
For your convenience we prepared a little library with useful components which
you can use here. Feel free to use your own components though and style your app
however you want!
...
// highlight-next-line
import { InCallButton } from "../components/InCallButton";
